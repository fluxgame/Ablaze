...
#    Exonio.pv((1+rate_of_return)/(1+inflation_rate)-1, (death_date - on_date)/365.25, annual_spending * -1, 0)
  ...
  def combined_tax_brackets
    [[0,8800],[0.051,24000],[0.151,43050],[0.171,101400],[0.271,189000],[0.291,339000],[0.371,424000],[0.401,624000],[0.421,999999999999999]]
  end
  
  def federal_tax_brackets
    [[0,24000],[0.1,43050],[0.12,101400],[0.22,189000],[0.24,339000],[0.32,424000],[0.35,624000],[0.37,999999999999999]]
  end

  def state_tax_brackets
    [[0,8800],[0.051,999999999999999]]
  end  
  ...
  def calculate_tax(gross_income, div_cap_gains)
    calculate_fed_tax(gross_income, div_cap_gains) + calculate_state_tax(gross_income, div_cap_gains)
  end
  
  def calculate_fed_tax(gross_income, div_cap_gains)
    walk_brackets(federal_tax_brackets, gross_income - div_cap_gains) + [[gross_income - federal_tax_brackets[2][1], 0].max, [0,div_cap_gains].max].min * 0.15
  end

  def calculate_state_tax(gross_income, div_cap_gains)
    walk_brackets(state_tax_brackets, gross_income)
  end
